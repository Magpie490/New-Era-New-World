NDefines = {
	...
	NDiplomacy = {
		...
		PAGAN_PEACE_MONTHS = 36,														-- Months before the Peace Prestige loss kicks in for certain Pagan religions
		...
		OFFER_VASSALIZATION_INTERACTION_PRESTIGE = 150,
		...
		ASK_FOR_VASSALIZATION_INTERACTION_PRESTIGE = 150,
		...
		BREAK_BETROTHAL_INTERACTION_MONEY = 150,
		BREAK_BETROTHAL_INTERACTION_PIETY = 0,
		BREAK_BETROTHAL_INTERACTION_PRESTIGE = 100,
		...
		ASK_FOR_DIVORCE_INTERACTION_PRESTIGE = 100,
		...
		DIVORCE_INTERACTION_PRESTIGE = 75,
		...
		CALL_ALLY_INTERACTION_THRESHOLD_FOR_YES = 50,
		...
		NOMINATE_BISHOP_TO_POPE_INTERACTION_PIETY = 50,
		...
		FORM_ALLIANCE_INTERACTION_PRESTIGE = 200,
		...
		OTHER_DEFENSIVE_PACT_JOIN_THREAT = 0.99,										-- If threat is this high, other religious groups band together when attacked, if it's lower every defensive pact acts on it's own
	...
	NCharacter = {
		...
		MUSLIM_NUM_WIVES_MONTHLY_PRESTIGE_PENALTY = 0.5,							-- The prestige effect from each lacking expected wife
		...
		MAX_JOINED_FACTIONS = 4,													-- Maximum number of Factions a character can be a member of (including led factions)
		...
		BASE_MAX_ATTRIBUTE = 13,													-- The max attribute value you can get from growing up
		...
		TREASURY_CHANCE_TO_DISAPPEAR_NO_HEIR = 0.25,									-- Chances that artifacts disappear on inheritance when the dying character doesn't have an heir
	},
	...
	-- Title Grant Decadence Reduction (Multiplied with current decadence)
		...
		GAME_RULES_DEJURE_LONG = 200,
		GAME_RULES_DEJURE_LONG_EMPIRE = 200,
		...
		MAX_REPUBLIC_COUNTIES_IN_REALM = 0.15,										-- 10% of a feudal realm is allowed to be under vassal republics (affects title grants)
		MAX_THEOCRACY_COUNTIES_IN_REALM = 0.15,										-- 10% of a feudal realm is allowed to be under vassal theocracies/bishoprics (affects title grants)
	...
	NReligion = {
		...
		ELECTOR_TITLE_CULTURE_FACTOR = 5,
	...
	NEconomy = {
		...
		TRADEZONE_VALUE_MULT = 0.4,													-- Multiple on trade zone value in tradezone bonus to trade posts and cities
		...
		TRADE_POST_COST_INC_DIST = 0.004,											-- The % increase in cost due to distance to closest connected province
		...
		PATRICIAN_CAMPAIGN_FUND_FACTOR = 4,											-- The effect of money in the Campaign Fund on Doge elections (money * factor)
		PATRICIAN_PRESTIGE_RESPECT_FACTOR = 4,										-- The effect of Prestige on the Respect value for Doge elections (prestige * factor)
		PATRICIAN_AGE_RESPECT_FACTOR = 1.5,											-- The effect of Age on the Respect value for Doge elections (age * age * factor)
		DOGE_SUCC_RANDOM_FACTOR = 750,												-- The random Respect factor on actual Doge succession
		MERCHANT_REPUBLIC_SALARY_MULT = 6,											-- Councillor salaries in Merchant Republics are multiplied by this
		...
		FORT_CONSUMED_IN_SETTLEMENT_CONTRUCTION = 0,								-- If set to 1 then fort holdings are consumed by the construction of another holding in the province while giving a discount to the construction cost in return.
	...
	NMilitary = {
		...
		MERCENARY_VASSAL_MAINTENANCE_FACTOR = 0.75,									-- Maintenance factor for VASSALIZED mercenary units
		...
		VASSAL_MERC_HIRE_COST_FACTOR = 0.15,										-- Fraction of nominal hire cost when hiring vassal mercenaries OR Holy Orders
		...
		DEFENDER_SIEGE_DAMAGE = 0.04,													-- Siege attack values are multiplied by this value(when not doing a sally), for defenders
		...
		CONTESTED_TERRITORY_WARSCORE_MULTIPLIER = 2.25,								-- Multiplier for contested settlements
		...
		FLANKING_DAMAGE_BONUS = 0.4,												-- Percentage bonus when flanking in combat
		...
		LIGHT_INFANTRY_PHASE_SKIRMISH_ATTACK = 1.33,
		...
		ARCHERS_PHASE_SKIRMISH_ATTACK = 2.5,
		...
		BOTTLENECK_DEFENDER_MARTIAL_MODIFIER = 1.5,
		...
		SHATTERED_RETREAT_FORBIDDEN_LOOT_MONTHS = 240,								-- How many months a character will be forbidden to raid an enemy after their armies shatter
	...
	NEngine = {
		EVENT_PROCESS_OFFSET = 200,													-- Events are checked every X day per character or province (1 is ideal, but CPU heavy). Max value of 255
		COURTIER_EVENT_PROCESS_OFFSET = 220,											-- Events are checked every X day for courtiers. Max value of 255
	...
	NFrontend = {
		...
		MAX_ZOOM_LEVEL = 3500,														-- How far out the player can zoom. Warning: At very high zoom levels, the ground will no longer render
